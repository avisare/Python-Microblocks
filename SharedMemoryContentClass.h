...
void SharedMemoryContentClassRunner(py::module & SharedMemoryWrapperModule)
{
	...
	py::class_<sm_data::SharedMemoryContent>(SharedMemoryWrapperModule, "SharedMemoryContent")
		...
		.def("paramsCtor", [](sm_data::SharedMemoryContent& obj, uint32_t setintData, py::list setcstringData, py::list settest)
	{
		obj.intData = setintData;
		...
		for (int i = 0; i < 2; i++)
		{
			py::list temp0 = settest[i].cast<py::list>();
			for (int j = 0; j < 2; j++)
			{
				obj.test[i][j] = temp0[j].cast<sm_data::t>();
			}
		}
	})
